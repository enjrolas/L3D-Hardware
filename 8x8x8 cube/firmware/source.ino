...
String modes="off;fireworks;zplasma;fftjoy;squarral";
String mode="fireworks";
int setMode(String _mode);
...
void setup() {
 ...
 pinMode(D7, OUTPUT);
 Serial.begin(115200);
 Serial.print("running system firmware version: ");
 Serial.println(System.version());
 if(strcmp(System.version(), FIRMWARE_VERSION)==0)
...
void publishCloudVariables()
{
	...
	Particle.variable("modes", modes);
	Particle.function("setMode", setMode);
	Particle.function("setAutocycle", setAutocycle);
}
...
int setAutocycle(String _loop)
{
	if(_loop.toLowerCase().compareTo("false")==0)
	autoCycle=false;
	else
	autoCycle=true;
}
...
void loop() {
  ...
  {
  digitalWrite(D7, (frameCount%10>5));
  } 
  ...
  if(fading)
    ...
    {
            if(mode.compareTo("fftjoy")!=0)
            cube.background(black);
            if(mode.compareTo("fireworks")==0)
                updateFireworks();
            if(mode.compareTo("zplasma")==0)
                zPlasma();
            if(mode.compareTo("squarral")==0)
                squarral();
            if(mode.compareTo("fftjoy")==0)
                FFTJoy();
        frameCount++;
...
void setFadeSpeed()
{
 ...
 if(mode.compareTo("fireworks")==0)
	mode="squarral";
 else if(mode.compareTo("squarral")==0)
      mode="zplasma";
 else if(mode.compareTo("zplasma")==0)
      mode="fftjoy";
 else if(mode.compareTo("fftjoy")==0)
      mode="fireworks";
     setFadeSpeed();
     fading=true;
 }
